...
int main(int argc, char** argv){
    ...
    image_transport::Subscriber img_sub = it.subscribe("/camera/infra1/image_rect_raw",1,imageCallback);
    ...
    std::cout << ie.GetVersions("GPU") << std::endl;


    cnnNetwork = ie.ReadNetwork("/home/zoo/frozen_darknet_yolov4_model.xml");
        ...
        else
            {
                 ...
                 if (OK == async_infer_request_curr->Wait(IInferRequest::WaitMode::RESULT_READY)) {
                     ...
                     if(cap_sign)
                     {
                        ...
                        cap_sign = false;
                        cv::imwrite("/home/zoo/compareB/"+std::to_string(cap_count)+".jpg",frame);
                        std::string filename = "/home/zoo/compareB/" + std::to_string(cap_count)+".txt";
                        std::ofstream file;
